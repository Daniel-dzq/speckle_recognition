...
# -----------------------------------------------------------------------------
# Paths (script lives in release_minimal/figures/)
# -----------------------------------------------------------------------------
...
def _find_repo_root() -> Path:
    for candidate in [THIS_DIR, *THIS_DIR.parents]:
        if (candidate / "experiments").is_dir() and (candidate / "figures").is_dir():
            return candidate
...
EXP_LEN = REPO_ROOT / "experiments" / "length_optimization"
EXP_LOSS = REPO_ROOT / "experiments" / "fiber_loss"

PRIMARY_TABLE = EXP_LEN / "outputs/fig3/Fig3_length_optimization_data.csv"
FALLBACK_TABLE = EXP_LEN / "outputs/length_optimization_green/tables/per_length_summary.csv"
POWER_CSV = EXP_LOSS / "data/power_loss.csv"
FIBER_LOSS_DIR = EXP_LOSS / "data"
OPTIMAL_JSON = EXP_LEN / "outputs/length_optimization_green/optimal_length.json"
CONFIG_YAML = EXP_LEN / "scripts/length_optimization_green.yaml"
LENGTH_OPTIMIZE_GREEN = EXP_LEN / "data/Green"

OUTPUT_DIR = REPO_ROOT / "figures" / "paper" / "Fig2_length_optimization"
...
def main() -> None:
    ...
    import os
    if os.environ.get("FIG2_INCLUDE_WORD", "").lower() in ("1", "true", "yes"):
