...
OUT_DIR = ROOT / "figures" / "new_datasets_analysis"
CACHE_ROOT = ROOT / ".analysis_cache"
...
def load_features(rel_dir: str, bucket: str, log: logging.Logger) -> Tuple[List[CaptureFeature], DatasetLayout]:
    layout = DatasetLayout.from_config(
        {
            "root": rel_dir,
            "layout": ("fiber_power_repeat" if rel_dir == "power_common_mode" else "flat_fiber_repeat"),
        },
        base_dir=ROOT,
    )
    caps = discover_captures(layout)
    log.info("%s: %d captures", rel_dir, len(caps))
    ...
    log.info("%s: %d feature rows", rel_dir, len(feats))
    return feats, layout
...
def main() -> None:
    ...
    log = logging.getLogger("new_datasets")
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    log.info("Output -> %s", OUT_DIR)

    fd, _ = load_features("disturbance_sensitivity", "disturbance_feats", log)
    fig_disturbance(fd, log)

    fl, _ = load_features("long_term_stability", "long_term_feats", log)
    fig_long_term(fl, log)

    fp, _ = load_features("power_common_mode", "power_feats", log)
    fig_power(fp, log)

    summary = collect_metrics_summary(fd, fl, fp)
    write_metrics_json(summary, OUT_DIR / "metrics_summary.json")
    log.info("Wrote %s", OUT_DIR / "metrics_summary.json")
