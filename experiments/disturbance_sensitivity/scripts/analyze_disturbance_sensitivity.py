...
def main() -> None:
    ...
    log = logging.getLogger("disturbance_sensitivity")
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    log.info("Data  -> %s", DATA_DIR)
    log.info("Output -> %s", OUT_DIR)

    fd, _ = load_features(DATA_DIR, "disturbance_feats", log)
    fig_disturbance(fd, log)

    summary = collect_metrics_summary(fd, [], [])
    write_metrics_json(
        {"disturbance_sensitivity": summary["disturbance_sensitivity"]},
